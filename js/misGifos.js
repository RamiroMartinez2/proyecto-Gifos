...
if (localStorage.getItem("misGifos") != null) {
  ...
  if (arregloMisGifos.length == 0) {
    ...
    document.getElementById("gif-con-contenido").style.display = "block";
    document.getElementById("ver-mas").style.display = "none";
  }
}

if (arregloMisGifos.length <= 12 && arregloMisGifos.length >= 1) {
  ...
  offsetFav = arregloMisGifos.length;
}
...
document.getElementById("create-gifo").addEventListener("mouseover", () => {
  document
    ...
    .setAttribute("src", "/assets/CTA-crear-gifo-hover.svg");
  document.getElementById("create-gifo").addEventListener("mouseout", () => {
    document
      .getElementById("create-gifo")
...
document.getElementById("create-gifo").addEventListener("click", () => {
  document
    .getElementById("create-gifo")
    .setAttribute("src", "/assets/CTA-crear-gifo-active.svg");
...
document.getElementById("create-gifo").addEventListener("click", misgifos);
...
function stopRecording() {
  ...
  document.getElementById("tiempo-repetir").style.borderBottom =
    "2px solid #5ED7C6";
  document
    .getElementById("tiempo-repetir")
  ...
  document.getElementById("subirGif").addEventListener("click", function () {
    ...
    fetch(
      `https://upload.giphy.com/v1/gifs?api_key=jAO1bZbtzM7A4mao2KGShgEF2qW9Ath0`,
      ...
      .then((json) => {
        ...
        hoverCard.appendChild(downloadButton);
        ...
        link.setAttribute("id", "copiador");

        link.addEventListener("mouseover", () => {
          link.src = "/assets/icon-link-hover.svg";
        });

        link.addEventListener("mouseout", () => {
          link.src = "/assets/icon-link-normal.svg";
        });
        link.setAttribute("src", "/assets/icon-link-normal.svg");

        hoverCard.appendChild(link);

        // function misLink() {
        for (i = 0; i < arregloMisGifos.length; i++) {
          let buscarId = arregloMisGifos[i];
          const apiId = `https://api.giphy.com/v1/gifs/${buscarId}?api_key=jAO1bZbtzM7A4mao2KGShgEF2qW9Ath0`;
          ...
          fetch(apiId)
            ...
            .then((json) => {
              ...
              span.setAttribute("id", "enlace");
              span = json.data.url;
              

              function copiar() {
                var inputFalso = document.createElement("input");
                inputFalso.setAttribute("value", span);
                console.log(inputFalso);
                document.body.appendChild(inputFalso);
                inputFalso.select();
                document.execCommand("copy");
                document.body.removeChild(inputFalso);
                subiendoGifo.textContent = "Enlace copiado con éxito";
              }

              document
                .getElementById("copiador")
        ...
        document
          ...
          .addEventListener("click", function () {
            ...
            document.getElementById("tiempo-repetir").style.visibility =
              "hidden";
            document.getElementById("tiempo-repetir").style.borderBottom =
              "none";
            document.getElementById("tiempo-repetir").innerHTML = "00:00:00";
...
let containerMisGifos = document.getElementById("mis-gifos");
  ...
  for (i = 0; i < arregloMisGifos.length; i++) {
    ...
    fetch(apiId)
      ...
      .then((json) => {
        ...
        img.addEventListener("click", abrirGifosMaxGifos);
        ...
        if (screen.width >= 641) {
          ...
          function borrar() {
            for (i = 0; i < arregloMisGifos.length; i++) {
              if (
                JSON.stringify(arregloMisGifos[i]) === JSON.stringify(newId)
              ) {
                arregloMisGifos.splice(i, 1);
                localStorage.setItem(
                  "misGifos",
                  JSON.stringify(arregloMisGifos)
                );
              }
            }
          }
...
document.getElementById("button-gif").addEventListener("click", () => {
